...
### 2026-09-18 — 下载链路与下载中心现代化

本次更新在上一轮 TUI 重构的基础上，重点完善下载可靠性、播放资源复用、下载中心信息架构以及歌单 / 音源页面的一致性：

- **修复下载误判失败**：下载响应即使 HTTP 200，也会校验 Content-Type 与音频内容；遇到 HTML、失效页面等非音频响应时自动放弃当前候选并继续换源，不再把错误页面保存成音频文件。
- **增强下载换源**：实际下载失败、响应不是音频或完整性校验失败时，按候选资源继续尝试；完整性错误仍保持严格失败策略，避免重复下载同一坏文件。
- **本地资源优先播放**：播放时优先复用已下载 / 自动缓存的本地文件，再进入 JS 音源和内置音源解析，减少重复网络请求。
- **现代化下载中心**：下载页增加进行中、等待、完成、失败摘要，并按任务状态分区展示进度、体积、码率和失败原因，保留取消、重试、清理等操作。
- **歌单页面层级优化**：歌单列表与歌单曲目使用独立焦点卡片，视觉层级与排行榜页面保持一致。
- **音源登录能力收敛**：登录入口根据音源 capabilities 动态显示；不支持登录的音源不再显示误导性的扫码入口。
- **文档与工程审计同步**：补充 Voicefox 与 go-musicfox 的功能 / 架构审计，明确后续优先级与不应引入的复杂度。

此前 2026-09-17 的 TUI 导航、播放控制和页面视觉重构继续保留。

### 2026-09-17 — TUI 交互与视觉重构

本次更新集中整理了播放器的全局导航、页面结构、播放控制与交互反馈：

- **重构全局导航**：重新整理侧边栏分组、页面入口和选中状态，统一各页面的导航行为与快捷键提示。
- **统一页面视觉语言**：抽取共享的页面 Chrome / Header 组件，减少重复边框，强化标题、分区和当前焦点的层级。
- **优化播放控制区**：重构底部播放控制组件，集中显示当前歌曲、播放控制、音量、播放模式、进度与音质等状态，避免重复出现多个底部状态栏。
- **优化队列与列表**：强化当前歌曲、选中行和列表信息的视觉层级，改善队列、收藏、历史、歌单等列表页的可读性。
- **完善搜索与排行榜**：统一音源标签、结果列表和页面提示，补齐页面切换与快捷键说明。
- **重构音源页面**：新增独立的音源管理页面，调整音源选择状态与列表布局，避免沿用设置页的展示方式。
- **扩展设置页面**：重新整理设置项与管理卡片，使配置内容能够按功能分区展示，并保留原有配置逻辑。
- **增强右键菜单**：为歌曲操作增加分组、图标、快捷键和禁用状态，降低操作菜单的信息密度。
- **删除旧进度条组件**：播放进度展示并入新的播放控制体系，避免同一状态在多个区域重复显示。
- **改善音源可靠性**：继续完善 JOOX Cookie、咪咕 H5、QQ vkey 等播放地址获取路径，并保留现有失败换源机制。
- **配置模型同步扩展**：为新的 TUI 状态与交互配置补充配置模型字段，并保持旧配置兼容。

本次更新不改变播放器核心架构，重点是提升终端窗口在不同尺寸下的信息密度、可读性和操作一致性。
...
| 音源说明 | [音源说明](#音源说明) |
| 项目结构 | [项目结构](#项目结构) |
| go-musicfox 对照审计 | [docs/GO-MUSICFOX-AUDIT.md](docs/GO-MUSICFOX-AUDIT.md) |
