...
所有配置项都支持在**设置页**（`0`）里直接修改，修改后会写回配置文件；键位自定义见本文[键位配置](#键位配置)，默认快捷键见 [KEYBINDINGS.md](../KEYBINDINGS.md)。
...
这些动作默认使用功能键和 `Shift+功能键`；数字键 `1` 到 `0` 保留给侧边栏标签页。可以在 `[keybindings.pages.settings]` 中逐项重绑。设置页会显示重绑后的按键，鼠标点击对应行与按键执行相同动作。
...
侧边栏导航固定使用 `1`～`0`：`1` 队列、`2` 搜索、`3` 排行榜、`4` 歌单、`5` 收藏、`6` 历史、`7` 本地音乐、`8` 下载、`9` 音源、`0` 设置。`Tab` / `Shift+Tab` 在这些页面之间循环切换。
